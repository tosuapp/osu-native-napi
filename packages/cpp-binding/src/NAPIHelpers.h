...
static int napi_is_int32(napi_env env, napi_value value, int *integer) {
  double temp = 0;
  if (
      // We get the value as a double so we can check for NaN, Infinity and
      // float: https://github.com/nodejs/node/issues/26323
      napi_get_value_double(env, value, &temp) != napi_ok ||
      // Reject NaN:
      isnan(temp) ||
      // Reject Infinity and avoid undefined behavior when casting double to
      // int: https://groups.google.com/forum/#!topic/comp.lang.c/rhPzd4bgKJk
...
#define NAPI_IS_INT32(valuetype, value)                                        \
  (NAPI_IS_NUMBER(valuetype) && napi_is_int32(env, value, nullptr))
#define NAPI_IS_UINT32(valuetype, value)                                       \
